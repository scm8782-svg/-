...
> **⚠️ 결론 (2026-09-01): 자동 갱신 운영 중단.**
> 사용량 조회에는 `user:profile` 스코프의 세션 토큰이 필요한데 8시간마다 만료되고,
> 갱신 엔드포인트는 수시로 429 로 차단되며, 장기 토큰(`claude setup-token`)에는
> 이 스코프가 부여되지 않는다(403). 즉 무인 자동 갱신을 Anthropic 이 사실상 막아둔 상태다.
> 세션 토큰을 수동으로 갈아넣으면 그동안은 동작한다. 공식 대안: Claude 앱 → 설정 → 사용량.
...
이후에는 30분마다 자동으로 갱신됩니다. 앱을 열면 1분마다 최신 데이터를 다시 확인하고, 새로고침 버튼으로 언제든 즉시 확인할 수 있습니다.

> GitHub 무료 cron 은 "최선 노력" 방식이라 실행이 밀리거나 건너뛸 수 있습니다.
> 5분(`*/5`) 처럼 짧은 간격은 아예 실행되지 않는 사례가 있어 30분으로 설정했습니다.
> 더 촘촘한 갱신이 필요하면 cron-job.org 같은 외부 스케줄러로
> `workflow_dispatch` 를 호출하는 방식이 확실합니다.
...
1. `VAPID_PRIVATE_KEY` Secret 등록 (푸시 서명용 개인키 — 최초 1회)
2. 폰에서 앱을 열고 **"🔔 한도 임박 알림" 카드 → 알림 켜기** → 권한 허용
3. 나오는 **구독 정보 복사** → `PUSH_SUBSCRIPTION` 이름으로 Secret 등록

임계값을 바꾸려면 `scripts/send-alerts.mjs`의 `THRESHOLDS`를 수정하세요.
알림을 끄려면 앱에서 "알림 끄기"를 누르거나 `PUSH_SUBSCRIPTION` Secret을 삭제하면 됩니다.
폰을 바꾸거나 브라우저 데이터를 지우면 구독이 무효화되니 2~3번을 다시 해주세요.
...
| 예약 실행이 아예 안 돎 | GitHub cron 은 보장되지 않습니다. 간격을 늘리거나(예: `"7 * * * *"`) 외부 스케줄러 사용 |
| 갱신 주기를 바꾸고 싶다 | `.github/workflows/usage.yml`의 `cron` 값 수정 — 429가 잦으면 간격을 늘려 보세요 |
